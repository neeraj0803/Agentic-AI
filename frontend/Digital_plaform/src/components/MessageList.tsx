...
function AnalysisSection({ title, items }: { title: string; items: string[] }) {
  return <section className="analysis-section"><h3>{title}</h3>{items.length > 0 ? <ul>{items.map((item, index) => <li key={`${title}-${index}`}>{item}</li>)}</ul> : <p className="analysis-empty">No items identified</p>}</section>
}

function ContextAnalysisResult({ analysis }: { analysis: ContextAnalysis }) {
  return <div className="context-analysis"><div className="analysis-heading"><span>Context analysis</span><span className="analysis-status">Complete</span></div><AnalysisSection title="Problem statement" items={[analysis.problem_statement]} /><AnalysisSection title="Business goal" items={[analysis.business_goal]} /><AnalysisSection title="Evidence" items={analysis.evidence} /><AnalysisSection title="Users" items={analysis.users} /><AnalysisSection title="Assumptions" items={analysis.assumptions} /><AnalysisSection title="Constraints" items={analysis.constraints} /></div>
}
